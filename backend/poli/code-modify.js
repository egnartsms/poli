...
releaseSnapshot ::= function (snapshot) {
...
prepareSnapshot ::= function (snapshot) {
   ...
   // For the purposes of module modification, we look whether the module changed or any
   // of its entries changed.
   let m2old = new Map;
...
rollbackToSnapshot ::= function (snapshot) {
...
commitSnapshot ::= function (snapshot) {
   $.releaseSnapshot(snapshot);
...
computeModificationsSince ::= function (snapshot) {
...
computeModuleModificationsSince ::= function* (mOld, mNew, isEntryChanged) {
